...
const getTimerInfo = (boxes) => {
  ...
  if (cur) {
    ...
    return { type: 'current', box: cur, remaining, progress };
  }
  const upcoming = boxes.filter(b => b.start > nowMin).sort((a, b) => a.start - b.start);
  if (upcoming.length > 0) {
    const next = upcoming[0];
    return { type: 'next', box: next, remaining: Math.max(0, (next.start - nowMin) * 60) };
  }
  return { type: 'idle' };
};
...
export default function App() {
  ...
  const addTask = () => {
    const id = newId('t');
    setFTasks(prev => [...prev, { id, text: '', done: false }]);
    setTimeout(() => document.getElementById(`task-input-${id}`)?.focus(), 30);
  };
  const addTaskAfter = (idx) => {
    const id = newId('t');
    setFTasks(prev => [...prev.slice(0, idx + 1), { id, text: '', done: false }, ...prev.slice(idx + 1)]);
    setTimeout(() => document.getElementById(`task-input-${id}`)?.focus(), 30);
  };
...
function EditView({
  C, boxes, displayRange, rangeConflicts, onSlotDown, onSlotEnter, onSlotLeave, onBoxClick,
          ...
          <div style={{ position: 'relative' }}>
            ...
            {boxes.map(box => {
              ...
              return (
                <div
                  key={box.id}
                  onMouseDown={(e) => onBoxClick(box, e)}
                  style={{
                    position: 'absolute',
                    ...
                    overflow: 'hidden',
                    zIndex: 10,
                    outline: isEditing ? `2px solid ${C.text}` : 'none',
                    outlineOffset: isEditing ? 2 : 0,
  ...
  return (
    <div style={{ backgroundColor: C.card, borderRadius: 12, border: `1px solid ${C.border}`, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '60px 24px 40px', textAlign: 'center' }}>
        {timer.type === 'current' && (
          <>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 12 }}>
              <div style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: timer.box.color }} />
              <div style={{ fontSize: 11, color: C.textMid, textTransform: 'uppercase', letterSpacing: '0.25em', fontWeight: 600 }}>진행 중</div>
            </div>
            <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 32, color: C.text }}>
              {timer.box.title}
            </div>
            <div className="dtb-tnum" style={{
            ...
            <div style={{ fontSize: 13, color: C.textMid, marginTop: 20, fontWeight: 600 }}>남은 시간</div>
        ...
        {timer.type === 'next' && (
          <>
            <div style={{ fontSize: 11, color: C.textMid, textTransform: 'uppercase', letterSpacing: '0.25em', fontWeight: 600, marginBottom: 12 }}>다음 일정까지</div>
            <div style={{ fontSize: 20, fontWeight: 700, marginBottom: 32, color: C.textMid, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              <div style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: timer.box.color }} />
              {timer.box.title}
            </div>
            <div className="dtb-tnum" style={{
            ...
            <div className="dtb-tnum" style={{ fontSize: 13, color: C.textMid, marginTop: 20, fontWeight: 600 }}>
