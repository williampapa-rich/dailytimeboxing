...
export default function App() {
  ...
  return (
    <div className="dtb-root" onClick={() => fabOpen && setFabOpen(false)} style={{ height: '100dvh', display: 'flex', flexDirection: 'column', overflow: mode === 'edit' ? 'hidden' : 'auto', backgroundColor: C.bg, color: C.text, colorScheme: C.scheme, position: 'relative', opacity: loading ? 0 : 1, transition: 'opacity 0.4s ease' }}>
      ...
      <div style={{ flex: 1, minHeight: 0, maxWidth: 1032, margin: '0 auto', padding: window.matchMedia?.('(max-width: 768px)')?.matches ? '4px 12px' : '16px 24px', width: '100%', position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', justifyContent: mode === 'view' ? (window.matchMedia?.('(max-width: 768px)')?.matches ? 'flex-start' : 'center') : 'stretch', transition: 'all 0.5s cubic-bezier(0.4, 0, 0.2, 1)' }}>
        {loading ? (
